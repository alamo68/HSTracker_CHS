...
// MARK: - Small AVG-DMG-style button right after Bob's Buddy panel

private final class ClashAvgStyleButtonView: NSView {
    var title: String = "一键拔线" {
        didSet {
            needsDisplay = true
        }
    ...
    override func draw(_ dirtyRect: NSRect) {
        // 只画与 AVG DMG 格子等高的上半部分（32pt），底部保持透明，
        // 使整体视觉高度与面板里的 AVG DMG 完全一致。
        let cellRect = NSRect(
            x: 0,
            y: bounds.height - 32,
            width: bounds.width,
        ...
        NSColor(
            calibratedRed: 0.0784313725490196,
            green: 0.0862745098039216,
        ...
        let attributes: [NSAttributedString.Key: Any] = [
            ...
            .foregroundColor: NSColor(
                calibratedRed: 0.541176470588235,
                green: 0.776470588235294,
                blue: 0.431372549019608,
                alpha: 1
            ),
            .paragraphStyle: paragraph,
        ]
...
final class ClashSkipCompanionController: NSObject {
    var onSkip: (() -> Void)?

    private let overlay = ClashSkipOverlayPanel(
        contentRect: NSRect(x: 0, y: 0, width: 70, height: 52),
        styleMask: [.borderless, .nonactivatingPanel],
        backing: .buffered,
        defer: false
    )
    private let buttonView = ClashAvgStyleButtonView()
    private var timer: Timer?
    private var resetFeedbackTask: DispatchWorkItem?
    ...
    private func configure() {
        buttonView.frame = overlay.contentView?.bounds ?? NSRect(x: 0, y: 0, width: 70, height: 52)
        buttonView.onClick = { [weak self] in
            self?.onSkip?()
        }
        overlay.contentView?.addSubview(buttonView)
    ...
    private func startPolling() {
        let timer = Timer(timeInterval: 0.2, target: self, selector: #selector(poll), userInfo: nil, repeats: true)
        self.timer = timer
        RunLoop.main.add(timer, forMode: .common)
    }

    @objc private func poll() {
        guard let panelWindow = AppDelegate.instance().coreManager?
                .game.windowManager.bobsBuddyPanel.window,
              panelWindow.isVisible else {
            hideOverlay()
            return
        }

        let panelFrame = panelWindow.frame
        // AVG DMG 区域之间本身用 5pt 分隔，这里保持同样的间隔。
        overlay.setFrame(
            NSRect(x: panelFrame.maxX + 5, y: panelFrame.minY, width: 70, height: 52),
            display: false
        )
        if !overlay.isVisible {
            overlay.orderFront(nil)
